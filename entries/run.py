...
class Run:
    def __init__(self, dicto: dict):
        self.gamecat: GameCate = GameCate(
            Game(dicto["game"], dicto["level"]),
            Category(dicto["category"], dicto["values"]),
        )
...
class PB(Run):
    def __init__(self, dicto: dict):
        ...
        self.leaderboard = LB(
            self.gamecat.game.release, dicto["place"], *self.gamecat.ids()
        )
    ...
    def __getitem__(self, clé):
        if clé == "WR":
            ...
            return (len(self.leaderboard) - self.leaderboard.place) / len(
                self.leaderboard
            )
